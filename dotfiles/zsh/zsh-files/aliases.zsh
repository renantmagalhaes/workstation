...
if [[ $macos_check == "darwin" ]]; then
    ...
    alias nordvpn-c='nordvpn connect && nordvpn whitelist add subnet 192.168.2.0/24 && nordvpn set tpl on && nordvpn set killswitch on && nordvpn set autoconnect on && nordvpn set lan-discovery enable'
    alias nordvpn-d='nordvpn set autoconnect off && nordvpn set killswitch off && nordvpn set tpl off && nordvpn disconnect && nordvpn set lan-discovery enable'
