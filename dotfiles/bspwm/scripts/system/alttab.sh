...
map_workspace_to_pair() {
	...
	local workspaces_monitor1=($(bspc query -D -m DisplayPort-1 --names))
	local workspaces_monitor2=($(bspc query -D -m DisplayPort-0 --names))
...
windows_on_desktops=$(
	bspc query -N -d "$focused_desktop" -n .window
	bspc query -N -d "$paired_desktop" -n .window
)
