#!/usr/bin/env bash
# Lock screen with blurred screenshot background per monitor for MangoWM
...
LOCK_OVERLAY_DIR="${LOCK_OVERLAY_DIR:-$HOME/.dotfiles/niri/lock-frames}"
LOCK_OVERLAY_FPS="${LOCK_OVERLAY_FPS:-16}"
LOCK_OVERLAY_CROSSFADE_MS="${LOCK_OVERLAY_CROSSFADE_MS:-45}"
LOCK_OVERLAY_SCRIPT="$HOME/.dotfiles/niri/scripts/lock_overlay_frame.sh"
...
HYPRLOCK_CONFIG=""
for config_path in "$HOME/.dotfiles/niri/hyprlock/hyprlock.conf"; do
...
HYPRLOCK_CONFIG="${HYPRLOCK_CONFIG:-${HYPRLOCK_CONFIG_ENV:-$HOME/.dotfiles/niri/hyprlock/hyprlock.conf}}"
...
if command -v niri >/dev/null 2>&1; then
    monitors=$(niri msg -j outputs | jq -r 'keys[]' 2>/dev/null || echo "")
fi

# Fallback if mmsg is not available
if [[ -z "$monitors" ]] && command -v wlr-randr >/dev/null 2>&1; then
    monitors=$(wlr-randr | awk '/^[A-Za-z0-9-]+/ {print $1}' || echo "")
fi
...
capture_monitor() {
    ...
    if command -v grim >/dev/null 2>&1; then
        ...
        if grim -o "$monitor" "$IMAGE_PATH" 2>/dev/null; then
            ...
            if grim "$IMAGE_PATH" 2>/dev/null; then
                captured=1
            fi
        fi
    fi
