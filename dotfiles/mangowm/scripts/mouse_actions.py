...
def get_wlr_randr_monitors():
    """Parse active monitors and their geometries using wlr-randr"""
    try:
        ...
        for line in out.splitlines():
            ...
            if not line.startswith(" ") and stripped:
                if current_mon and current_mode and pos:
                    monitors.append({
                        "name": current_mon,
                ...
                if parts:
                    res = parts[0].split("x")
                    if len(res) == 2:
                        current_mode = (int(res[0]), int(res[1]))
        ...
        logging.error(f"Failed to get wlr-randr monitors: {e}")
        return []


def clip_to_monitors(x, y, monitors):
    """Clip coordinates to the active monitor layout boundaries"""
    if not monitors:
        return x, y
    ...
    for m in monitors:
        mx, my, mw, mh = m["x"], m["y"], m["width"], m["height"]
        cx = max(mx, min(x, mx + mw - 1))
        cy = max(my, min(y, my + mh - 1))
        dist = (x - cx) ** 2 + (y - cy) ** 2
        if dist < min_dist:
            min_dist = dist
            best_x, best_y = cx, cy
...
def get_selected_monitor_name():
    """Get the active/selected monitor name using mmsg"""
    try:
        ...
        logging.error(f"Failed to get selected monitor name: {e}")
    return None


def cursor_xy():
    """Get the tracked cursor position"""
...
class MonitorMap:
    ...
    def refresh(self, max_age=5.0):
        ...
        if now - self._last > max_age or not self._cache:
            try:
                self._cache = get_wlr_randr_monitors()
                self._last = now
            except Exception as e:
...
async def main():
    ...
    default_change_ws = os.path.join(scripts_dir, "change_workspace.sh")

    # Edge commands
    ap.add_argument(
        "--top-cmd-up",   default=f"{default_change_ws} right")
    ap.add_argument(
        "--top-cmd-down", default=f"{default_change_ws} left")
    ap.add_argument("--bottom-cmd-up",
                    default=f"{default_change_ws} right")
    ap.add_argument("--bottom-cmd-down",
                    default=f"{default_change_ws} left")
    ...
    # Initialize pointer tracking position
    global cursor_x, cursor_y
    monitors = monmap.get_monitors()
    sel_name = get_selected_monitor_name()
    init_mon = None
    if sel_name:
        for m in monitors:
            if m["name"] == sel_name:
                init_mon = m
                break
    if not init_mon and monitors:
        init_mon = monitors[0]
    if init_mon:
        cursor_x = init_mon["x"] + init_mon["width"] // 2
        cursor_y = init_mon["y"] + init_mon["height"] // 2
        logger.info(f"Initialized virtual cursor to center of {init_mon['name']}: {cursor_x}, {cursor_y}")
    else:
        cursor_x = 1280
        cursor_y = 720
        logger.warning(f"No monitors found. Initialized virtual cursor to {cursor_x}, {cursor_y}")

    async for ev in dev.async_read_loop():
        # ---------- Accumulate relative movements to track pointer ----------
        if ev.type == EC.EV_REL and ev.code in (EC.REL_X, EC.REL_Y):
            if ev.code == EC.REL_X:
                cursor_x += ev.value
            elif ev.code == EC.REL_Y:
                cursor_y += ev.value
            cursor_x, cursor_y = clip_to_monitors(cursor_x, cursor_y, monmap.get_monitors())
            continue
