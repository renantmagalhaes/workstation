...
LOCK_OVERLAY_DIR="${LOCK_OVERLAY_DIR:-$HOME/.dotfiles/mangowm/lock-frames}"
LOCK_OVERLAY_FPS="${LOCK_OVERLAY_FPS:-16}"
LOCK_OVERLAY_CROSSFADE_MS="${LOCK_OVERLAY_CROSSFADE_MS:-45}"
LOCK_OVERLAY_SCRIPT="$HOME/.dotfiles/mangowm/scripts/lock_overlay_frame.sh"
...
HYPRLOCK_CONFIG=""
for config_path in "$HOME/.dotfiles/mangowm/hyprlock/hyprlock.conf"; do
...
HYPRLOCK_CONFIG="${HYPRLOCK_CONFIG:-${HYPRLOCK_CONFIG_ENV:-$HOME/.dotfiles/mangowm/hyprlock/hyprlock.conf}}"
...
if command -v mmsg >/dev/null 2>&1; then
    monitors=$(mmsg -O 2>/dev/null || echo "")
fi
